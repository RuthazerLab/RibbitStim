...
Presents moving bar stimulus in 4 directions, in order
...
driftFreq = baselineFreq;
repeats = baselineRepeats;
stimtimes = zeros(4*repeats,3); %Timing data: stim ID + start time + end time
%Stim IDs: 1=AP, 2=PA, 3=Up, 4=Down
stimInterval = baselineISI;

exptime = initpause + baselineRepeats * ((1/driftFreq) + baselineISI) * 4;
...
message=sprintf(['Baseline recording: moving bars in 4 directions']);
disp(message);
fprintf(logtxt,[message '\n']);
message=sprintf(['Facing = ' facing]);
disp(message);
fprintf(logtxt,[message '\n']);
message=sprintf(['Baseline Freq= ' num2str(baselineFreq)]);
disp(message);
fprintf(logtxt,[message '\n']);
message=sprintf(['Baseline ISI = ' num2str(baselineISI)]);
disp(message);
fprintf(logtxt,[message '\n']);
...
for i = 1:baselineRepeats

    driftDir = 'AP';
    stimIndex = i;
    toctime = toc(start);
    message=sprintf(['[' num2str(toctime) ']: AP bar']);
    disp(message);
    fprintf(logtxt,[message '\n']);
    stimtimes(stimIndex,1)=1;
    ...
    stimIndex = i+1;
    toctime = toc(start);
    message=sprintf(['[' num2str(toctime) ']: PA bar']);
    disp(message);
    fprintf(logtxt,[message '\n']);
    stimtimes(stimIndex,1)=1;
    stimtimes(stimIndex,2)=toctime;
    ...
    stimIndex = i+2;
    toctime = toc(start);
    message=sprintf(['[' num2str(toctime) ']: Up bar']);
    disp(message);
    fprintf(logtxt,[message '\n']);
    stimtimes(stimIndex,1)=1;
    stimtimes(stimIndex,2)=toctime;
    ...
    stimIndex = i+3;    
    toctime = toc(start);
    message=sprintf(['[' num2str(toctime) ']: Down bar']);
    disp(message);
    fprintf(logtxt,[message '\n']);
    stimtimes(stimIndex,1)=1;
    stimtimes(stimIndex,2)=toctime;
...
save(datmat,'stimtimes','facing','baselineFreq','baselineISI','barsize','bartype');
